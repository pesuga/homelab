...
@router.put("/prompts/core")
async def update_core_prompt(
    request: Dict[str, str],
    prompt_builder: PromptBuilder = Depends(get_prompt_builder)
):
    """Update core system prompt (FAMILY_ASSISTANT.md)"""
...
@router.get("/prompts/roles")
async def list_roles(
    prompt_builder: PromptBuilder = Depends(get_prompt_builder)
):
    """List available roles"""
    try:
        return {"roles": prompt_builder.list_roles()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to list roles: {str(e)}")


@router.put("/prompts/role/{role}")
async def update_role_prompt(
    role: str,
    request: Dict[str, str],
    prompt_builder: PromptBuilder = Depends(get_prompt_builder)
):
    """Update role-specific prompt"""
    content = request.get("content")
    if not content:
        raise HTTPException(status_code=400, detail="Content is required")

    try:
        success = prompt_builder.update_role_prompt(role, content)
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to update role prompt for {role}")
        
        return {"message": f"Role prompt for {role} updated successfully"}

    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update role prompt: {str(e)}"
        )
...
async def get_skill_prompt(
    ...
    try:
        # We need to expose load_skill_prompt in PromptBuilder or use load_file directly
        # For now let's assume we can use load_skill_prompts with a single item list
        prompt = prompt_builder.load_skill_prompts([skill])
        
        if not prompt:
             raise HTTPException(status_code=404, detail=f"Skill prompt not found: {skill}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load skill prompt: {str(e)}"
        )


@router.put("/prompts/skill/{skill}")
async def update_skill_prompt(
    skill: str,
    request: Dict[str, str],
    prompt_builder: PromptBuilder = Depends(get_prompt_builder)
):
    """Update skill-specific prompt"""
    content = request.get("content")
    if not content:
        raise HTTPException(status_code=400, detail="Content is required")

    try:
        success = prompt_builder.update_skill_prompt(skill, content)
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to update skill prompt for {skill}")
        
        return {"message": f"Skill prompt for {skill} updated successfully"}

    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update skill prompt: {str(e)}"
        )
